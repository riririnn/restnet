...
TSUME_POSITIONS: list[dict] = [
    ...
    # ── 実戦型：美濃囲い 寄せの局面（12駒）────────────────────────────────────
    # 後手美濃（桂9一・玉8一・金7一 / 金9二・銀8二・歩7二 / 歩9三・歩8三・歩7三）
    # vs 先手竜7三・金8四・銀7四の包囲陣。
    # 先手持ち駒：金1枚。G*8b（頭金）で寄せが確定する局面。
    {
        "name": "実戦型：美濃囲い 寄せ（12駒）",
        "sfen": "nkg6/gsp6/pp+R6/1GS6/9/9/9/9/4K4 b G 1",
        "answer_usi": "G*8b",
        "description": (
            "後手が美濃囲い（桂・玉・金 / 金・銀・歩3枚）を構え、\n"
            "先手の竜7三・金8四・銀7四が迫る寄せの局面（盤上12駒）。\n"
            "G*8b（金を8二に打ち頭金）で後手玉を追い詰める最強攻め。\n"
            "AttentionMap: 竜と金銀が連携して囲いを崩す構造をモデルが捉えるか確認。"
        ),
    },
    # ── 実戦型：振り飛車終盤（37駒）────────────────────────────────────────────
    # 振り飛車vs居飛車の終盤。後手玉が5一に残り、先手は角を持ち駒に持つ。
    # B*4b（角を4二に打ち込む）が後手陣を崩す急所の一手。
    {
        "name": "実戦型：振り飛車終盤（37駒）",
        "sfen": "lnsgk2nl/1r4gs1/p1pppp1p1/1p6p/6P2/P1P1PS2P/1P1PP3P/2G4R1/LNS1KG1NL b Bb2p 25",
        "answer_usi": "B*4b",
        "description": (
            "振り飛車 vs 居飛車の終盤局面（盤上37駒）。\n"
            "後手玉は5一に留まり、先手は角を持ち駒に持つ。\n"
            "B*4b（角を4二に打ち込む）で後手陣の金銀を崩す急所手。\n"
            "AttentionMap: 多駒盤面で後手玉周辺と角の打ち込み先に注目するか確認。"
        ),
    },
    # ── 実戦型：相居飛車 竜が後手陣に侵入（41駒）──────────────────────────────
    # 相居飛車の終盤。先手竜が後手陣に侵入し寄せに入った局面。
    # 5h4h（先手銀を5六から4六へ進出）で攻めを加速する。
    {
        "name": "実戦型：相居飛車 竜侵入（41駒）",
        "sfen": "ln2k1snl/1r1sg1gb1/p1pppp1p1/1p4p1p/9/2P1SP1P1/PP1PP1P1P/1BG2+R1R1/LNS1KG1NL b Pp 21",
        "answer_usi": "5h4h",
        "description": (
            "相居飛車の終盤局面（盤上41駒）。先手竜が後手陣4段目に侵入済み。\n"
            "5h4h（銀を5八→4八へ）で竜のサポートと王への圧力を高める。\n"
            "両陣ともほぼ全駒が残る最も実戦に近い局面。\n"
            "AttentionMap: 多駒盤面で竜の位置と後手玉周辺への注目パターンを確認。"
        ),
    },
