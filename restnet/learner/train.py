...
from minizero.tools.analysis import analysis
...
class MinizeroDadaLoader:
    def __init__(self, conf_file_name):
        ...
        # allocate memory
        self.sampled_index = np.zeros(restnet_py.get_batch_size() * 2, dtype=np.int32)
        self.features = np.zeros(
            restnet_py.get_batch_size()
            * restnet_py.get_nn_num_input_channels()
            * restnet_py.get_nn_input_channel_height()
            * restnet_py.get_nn_input_channel_width(),
            dtype=np.float32,
        )
        self.loss_scale = np.zeros(restnet_py.get_batch_size(), dtype=np.float32)
        self.value_accumulator = (
            np.ones(1)
            if restnet_py.get_nn_discrete_value_size() == 1
            else np.arange(
                -int(restnet_py.get_nn_discrete_value_size() / 2),
                int(restnet_py.get_nn_discrete_value_size() / 2) + 1,
            )
        )
        if restnet_py.get_nn_type_name() == "alphazero":
            self.action_features = None
            self.policy = np.zeros(
                restnet_py.get_batch_size() * restnet_py.get_nn_action_size(),
                dtype=np.float32,
            )
            self.value = np.zeros(
                restnet_py.get_batch_size() * restnet_py.get_nn_discrete_value_size(),
                dtype=np.float32,
            )
            self.reward = None
        else:
            self.action_features = np.zeros(
                restnet_py.get_batch_size()
            ...
            self.policy = np.zeros(
                restnet_py.get_batch_size()
                * (restnet_py.get_muzero_unrolling_step() + 1)
    ...
    def load_data(self, training_dir, start_iter, end_iter):
        for i in range(start_iter, end_iter + 1):
            ...
            if len(self.data_list) > restnet_py.get_zero_replay_buffer():
    ...
    def sample_data(self, device="cpu"):
        ...
        action_features = (
            ...
            else torch.FloatTensor(self.action_features)
            .view(
                restnet_py.get_batch_size(),
                -1,
                restnet_py.get_nn_num_action_feature_channels(),
        ...
        policy = (
            torch.FloatTensor(self.policy)
            .view(restnet_py.get_batch_size(), -1, restnet_py.get_nn_action_size())
            .to(device)
        )
        value = (
            torch.FloatTensor(self.value)
            .view(
                restnet_py.get_batch_size(), -1, restnet_py.get_nn_discrete_value_size()
            )
            .to(device)
        )
        reward = (
            ...
            else torch.FloatTensor(self.reward)
            .view(
                restnet_py.get_batch_size(), -1, restnet_py.get_nn_discrete_value_size()
            )
            .to(device)
        )
        loss_scale = torch.FloatTensor(self.loss_scale / np.amax(self.loss_scale)).to(
            device
        )
...
def calculate_loss(network_output, label_policy, label_value, label_reward, loss_scale):
    # policy
    if restnet_py.use_gumbel():
        loss_policy = (
            nn.functional.kl_div(
                nn.functional.log_softmax(network_output["policy_logit"], dim=1),
                label_policy,
                reduction="none",
            ).sum(dim=1)
            * loss_scale
    ...
    if restnet_py.get_nn_discrete_value_size() == 1:
        loss_value = (
            nn.functional.mse_loss(
                network_output["value"], label_value, reduction="none"
            )
            * loss_scale
        ).mean()
    else:
        loss_value = -(
            (
                label_value
                * nn.functional.log_softmax(network_output["value_logit"], dim=1)
            ).sum(dim=1)
            * loss_scale
        ).mean()
    ...
    if label_reward is not None and "reward_logit" in network_output:
        loss_reward = -(
            (
                label_reward
                * nn.functional.log_softmax(network_output["reward_logit"], dim=1)
            ).sum(dim=1)
            * loss_scale
        ).mean()
...
def train(model, training_dir, data_loader, start_iter, end_iter):
    ...
    training_info = {}
    for i in range(1, restnet_py.get_training_step() + 1):
        ...
        (
            ...
            loss_scale,
            sampled_index,
        ) = data_loader.sample_data(model.device)

        if restnet_py.get_nn_type_name() == "alphazero":
            network_output = model.network(features)
            loss_policy, loss_value, _ = calculate_loss(
                network_output, label_policy[:, 0], label_value[:, 0], None, loss_scale
            )
            loss = loss_policy + restnet_py.get_value_loss_scale() * loss_value
        ...
        elif restnet_py.get_nn_type_name() == "muzero":
            network_output = model.network(features)
            ...
            for i in range(restnet_py.get_muzero_unrolling_step()):
                network_output = model.network(
                    network_output["hidden_state"], action_features[:, i]
                )
                ...
                loss_policy += loss_step_policy / restnet_py.get_muzero_unrolling_step()
                loss_value += loss_step_value / restnet_py.get_muzero_unrolling_step()
                loss_reward += loss_step_reward / restnet_py.get_muzero_unrolling_step()
                network_output["hidden_state"].register_hook(lambda grad: grad / 2)
            if restnet_py.use_per():
                data_loader.update_priority(sampled_index, batch_values)
            loss = (
                loss_policy
                + restnet_py.get_value_loss_scale() * loss_value
                + loss_reward
            )
...
if __name__ == "__main__":
    ...
    while True:
        try:
            command = input()
            command_prefix = command.split(" ", 1)[0]
            ...
            if command_prefix == "update_config":
                ...
                if not restnet_py.load_config_string(conf_str):
