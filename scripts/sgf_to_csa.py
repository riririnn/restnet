...
Action ids are decoded by scripts/shogi_coords.py (the shared, empirically
verified encoding), and this script tracks its own lightweight board (piece
...
from shogi_coords import (  # noqa: E402
    DROP_ID_TO_KIND, KIND_CSA, PROMOTED_CSA, decode_action,
)
...
class Board:
    ...
    def apply(self, mover, decoded):
        ...
        if decoded[0] == "drop":
            _, drop_piece_id, file_to, rank_to = decoded
            kind = DROP_ID_TO_KIND[drop_piece_id]
...
def game_to_csa(action_pairs, result=None, cap=0):
    ...
    for player_char, action_id in action_pairs:
        decoded = decode_action(action_id, player_char == "B")
        if decoded is None:
...
def collect_sgf_files(path):
    """Accept a single .sgf file or a directory; return the list of .sgf files."""
    if os.path.isdir(path):
        files = sorted(glob.glob(os.path.join(path, "*.sgf")))
        if not files:
            sys.exit(f"error: no .sgf files found in directory: {path}")
        return files
    return [path]
...
def main():
    ...
    ap.add_argument("sgf_path", help="minizero game-record file, or a directory of "
                                     ".sgf files (e.g. TRAIN_DIR/sgf/5.sgf or TRAIN_DIR/sgf/)")
    ...
    sgf_files = collect_sgf_files(args.sgf_path)
    if len(sgf_files) > 1 and not args.out_dir:
        sys.exit("error: -o/--out_dir is required when converting a directory")
    ...
    if args.out_dir:
        ...
        for sgf_file in sgf_files:
            with open(sgf_file) as f:
                games = parse_sgf_records(f.read())
            base = os.path.splitext(os.path.basename(sgf_file))[0]
            ...
            total += len(games)
        print(f"wrote {total} CSA file(s) from {len(sgf_files)} SGF file(s) "
              f"to {args.out_dir}", file=sys.stderr)
    else:
        with open(sgf_files[0]) as f:
            games = parse_sgf_records(f.read())
